...
class preProcessorTest(TestCase):
    ...
    def setUpClass(self):
        ...
        self.impl_pre_processor  = pp(self.model_name)
        ...
        assert self.impl_pre_processor.get_size() == 0
        assert self.impl_pre_processor.get_raw_data() == []
        logging.info("1:---최초 생성 테스트 완료---")
        logging.info("2:---샘플 데이터 입력 테스트---")
        
        self.impl_pre_processor.read_data(data_path=self.data_path, data_DOM=self.data_DOM)
        assert self.impl_pre_processor.get_size != 0
        assert self.impl_pre_processor.get_raw_data() != []
        assert len(self.impl_pre_processor.get_raw_data()) == self.impl_pre_processor.get_size()
        
        logging.info("현재 분류된 기사 개수: " + str(self.impl_pre_processor.get_size()))
        logging.info("현재 분류된 문장 개수: " + str(self.impl_pre_processor.get_context_size()))
        logging.info(self.impl_pre_processor.get_raw_data()[0])
    ...
    def test_remove_special_characters(self):
        ...
        clean_dataset = list(map(self.impl_pre_processor.remove_special_characters, test_dataset))
        self.assertEqual(clean_dataset, test_answer)
    
    @Test("마스크")
    def test_masking(self):
        sample_context = self.impl_pre_processor.get_raw_data()[0]
        token_context = self.impl_pre_processor.tokenizer(sample_context)
        mask_context = self.impl_pre_processor.masking(token_context)
        # num = random.randrange(0, len(maskContext))
        ratio_sum = 0.0

        for context in mask_context['input_ids']:
            num_mask = context.count(self.impl_pre_processor.tokenizer.mask_token_id)
            num_pad = context.count(self.impl_pre_processor.tokenizer.pad_token_id)
    ...
    def test_next_sentence_prediction(self):
        if self.impl_pre_processor.get_size() == self.impl_pre_processor.get_context_size():
            logging.warning('NSP 예측에 사용할 수 없는 데이터셋입니다.')
            return

        context_size = self.impl_pre_processor.get_context_size()
        
        #NSPmodule 기본값
        self.impl_pre_processor.nsp_mode.prob = 0.5
        self.assertEqual(self.impl_pre_processor.nsp_mode.prob, 0.5)

        # 잘못된 NSP 전략은 무시해야 한다
        base_strategy = self.impl_pre_processor.nsp_mode.get_strategy()
        self.assertFalse(self.impl_pre_processor.nsp_mode.set_strategy("no_strategy"))
        self.assertEqual(self.impl_pre_processor.nsp_mode.get_strategy(), base_strategy)


        test_size = context_size // 5
        nsp_result = self.impl_pre_processor.next_sentence_prediction(test_size)
        ...
        self.impl_pre_processor.nsp_mode.prob = 1
        nsp_result = self.impl_pre_processor.next_sentence_prediction(test_size)
        ...
        self.impl_pre_processor.nsp_mode.prob = 0

        # 다양한 문장 선택 전략 테스트
        # OnlyFirst는 오직 첫번째 문장(판별 대상 기본 문장) 기준으로 중복을 검사합니다.
        # 중복 여부 중요성이 적은 데이터를 여러번 사용하여 더 적은 데이터를 효과적으로 사용하기 위한 전략입니다.
        self.assertTrue(self.impl_pre_processor.nsp_mode.set_strategy("only_first"))
        test_size = context_size // 5
        nsp_result = self.impl_pre_processor.next_sentence_prediction(test_size)
        ...
        # TODO: 함수 구조 최적화 필요
        # self.assertTrue(self.impl_pre_processor .nsp_mode.setStrategy("soft"))
        # test_size  = 100
        # nspResult = self.impl_pre_processor .nextSentencePrediction(test_size )
        
        # self.assertEqual(test_size , len(nspResult))
